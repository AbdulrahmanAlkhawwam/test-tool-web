...
import { buildScope, MAX_SCOPE_CASE_IDS, scopeError, type ScopeState } from './run-scope';
...
describe('automated run scope', () => {
  ...
  it('caps the number of selected test cases', () => {
    const caseIds = Array.from({ length: MAX_SCOPE_CASE_IDS }, (_, i) => `c${i}`);
    expect(scopeError({ ...base, mode: 'CASES', caseIds }, 'e2e')).toBeNull();
    expect(scopeError({ ...base, mode: 'CASES', caseIds: [...caseIds, 'extra'] }, 'e2e')).toBe(
      `Choose at most ${MAX_SCOPE_CASE_IDS} test cases`,
    );
  });
